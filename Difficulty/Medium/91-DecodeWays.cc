...
class Solution
{
  public:
    int numDecodings(string s)
    {
        ...
        int prev = 0;
        int cur = 1; // s[0] != 0 则从第1位初始时有1种解码方法

        // 长度为n的字符串，有n+1个阶梯
        for (int i = 1; i <= n; i++)
        {
            // 匹配到s[i-1]位置，cur代表长度为i的字符串对应的解码方法数
            if (s[i - 1] == '0') // 如s[i-1]=0，则s[i]只能联合前一位进行解码，不能单独解码
                // 当前cur计算单独解码数
                cur = 0;
            if (i < 2 || !(s[i - 2] == '1' || (s[i - 2] == '2' && s[i - 1] <= '6'))) // 前面无字符i=1 或 (前面字符不是1 且 前面字符串不是2或当期字符不是1)，不能联合解码
                // 当前prev计算联合解码数
                prev = 0;
...
// 动态规划
// dp[i]表示字符串长度为i时的解码方法数
// dp[s.size()]作为返回值，表示参数字符串s的解码方法数
class Solution
{
  public:
    int numDecodings(string s)
